...
// and then register another one?
	...
	switch (cindex) {
	case 0:
		input_set_abs_params(map->input, ABS_X, ABSMIN_ACC_VAL, ABSMAX_ACC_VAL, 0, 0);
		break;
	case 1:
		input_set_abs_params(map->input, ABS_Y, ABSMIN_ACC_VAL, ABSMAX_ACC_VAL, 0, 0);
		break;
	case 2:
		input_set_abs_params(map->input, ABS_Z, ABSMIN_ACC_VAL, ABSMAX_ACC_VAL, 0, 0);
		break;
	}
	mutex_unlock(&inputbridge_device_mutex);

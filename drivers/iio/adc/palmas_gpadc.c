...
static int palmas_gpadc_start_mask_interrupt(struct palmas_gpadc *adc, bool mask)
{
	int ret;
...
static int palmas_gpadc_probe(struct platform_device *pdev)
{
	...
	if (gpadc_pdata->iio_maps) {
		ret = iio_map_array_register(indio_dev, gpadc_pdata->iio_maps);
		if (ret < 0) {
...
static int palmas_adc_wakeup_configure(struct palmas_gpadc *adc)
{
	...
	if (adc->wakeup1_enable) {
		...
		if (adc->wakeup1_data.adc_high_threshold > 0) {
			...
			threshold_polarity = PALMAS_GPADC_THRES_CONV0_MSB_THRES_CONV0_POL;
		}
		...
		ret = palmas_write(adc->palmas, PALMAS_GPADC_BASE,
				...
				((thres >> 8) & 0xF) | threshold_polarity);
		if (ret < 0) {
			dev_err(adc->dev,
	...
	if (adc->wakeup2_enable) {
		...
		if (adc->wakeup2_data.adc_high_threshold > 0) {
			...
			threshold_polarity = PALMAS_GPADC_THRES_CONV1_MSB_THRES_CONV1_POL;
		}

...
/* Trace macros*/
#define NFC_DRV_ERR(fmt, arg...)	pr_err("nfcdrv: "fmt"\n" , ## arg)
#define NFC_DRV_INFO(fmt, arg...)	pr_info("nfcdrv: "fmt"\n" , ## arg)
#define NFC_DRV_DBG(fmt, arg...)	pr_debug("nfcdrv: "fmt"\n" , ## arg)
#define NFC_DRV_FUNC_START()		pr_debug("nfcdrv: Start %s\n", __func__)
...
struct nfc_dev {
	atomic_t available;		/* for single openness */
	int major;
	int minor;
	struct class *cls;
...
static int nfc_drv_open(struct inode *inode, struct file *filp)
{
	...
	if (unlikely(!atomic_dec_and_test(&nfc_device->available))) {
		NFC_DRV_ERR("already open");
		ret = -EBUSY;
		goto release_exit;
	}
	...
	if (unlikely(!dev)) {
		NFC_DRV_ERR("unable to allocate nfc_dev_data");
		ret = -ENOMEM;
		goto release_exit;
	}
	...
	NFC_DRV_DBG("st_register returned %d", ret);
	...
	if (unlikely(ret < 0)) {
		if (ret == -EINPROGRESS) {
			...
			if (comp_ret == 0) {
				...
				NFC_DRV_ERR("st_register_cb failed %d", ret);
				goto free_exit;
			}
		} else {
			goto free_exit;
...
release_exit:
	atomic_inc(&nfc_device->available); /* release the device */
			...
			size_t count, loff_t *f_pos)
{
	...
	if (unlikely(!skb)) {
		NFC_DRV_ERR("unable to allocate skb");
		ret = -ENOMEM;
		goto exit;
	}
	...
	if (unlikely(copy_from_user(skb_put(skb, count), buf, count))) {
		NFC_DRV_ERR("cannot copy tx from user space to kernel space");
		ret = -EFAULT;
		goto free_exit;
	}
...
exit:
	NFC_DRV_DBG("nfc_drv_write returning %d", ret);
	return ret;
}
			...
			size_t count, loff_t *f_pos)
{
	...
	NFC_DRV_DBG("recv new rx skb, count %d, skb->len %d", count, skb->len);
	...
	if (unlikely(copy_to_user(buf, skb->data, ret))) {
		NFC_DRV_ERR("cannot copy rx from kernel space to user space");
...
exit:
	NFC_DRV_DBG("nfc_drv_read returning %d", ret);
	return ret;
}
...
static int __init nfc_drv_init(void)
{
	...
	if (unlikely(IS_ERR(nfc_device->cls))) {
		NFC_DRV_ERR("class_create failed");
	...
	if (unlikely(IS_ERR(sysfs_dev))) {
		NFC_DRV_ERR("device_create failed");
		ret = PTR_ERR(sysfs_dev);
		goto class_exit;
	}

	NFC_DRV_INFO("allocated major %d, minor %d", nfc_device->major,
					nfc_device->minor);

...
{
	REG_SADC_SAMETIME = 350;  /* about 3.5 ms */
	REG_SADC_WAITTIME = 50;  /* about 0.5 ms */
...
static int __init sadc_init(void)
{
	...
	sadc_buf = (unsigned char *)__get_free_pages(GFP_KERNEL, 4);
	if (!sadc_buf) {
...
static void __exit sadc_exit(void)
{
	...
	free_pages((unsigned long)sadc_buf, 4);
	free_irq(IRQ_SADC, dev);
	jz_unregister_chrdev(SADC_MINOR, SADC_NAME);
	kfree(dev);
}

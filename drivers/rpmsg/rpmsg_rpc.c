...
/*
 * idr utility function, define this way for easier replacement in 3.9+ kernel
 * min & max are currently unused, but aligns with the newer definition.
 */
static int idr_alloc(struct idr *idr, void *ptr, int min, int max, gfp_t mask)
{
	int id;
	int ret;

	do {
		if (!idr_pre_get(idr, mask))
			return -ENOMEM;

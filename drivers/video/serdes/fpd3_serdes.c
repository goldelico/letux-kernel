...
{
	return __fpd3_serdes_dirval(chip, offset, DIR_INPUT, SHIFT_DIR);
}
		...
		unsigned offset, int value)
{
	...
	ret = __fpd3_serdes_dirval(chip, offset, DIR_OUTPUT, SHIFT_DIR);

	if (ret)
		ret = __fpd3_serdes_dirval(chip, offset, value, SHIFT_VAL);
...
{
	__fpd3_serdes_dirval(chip, offset, value, SHIFT_VAL);
}
...
int fpd3_serdes_initialize(struct i2c_client *client)
{
	...
	if (data->slave_mode == false) {
		...
		while (ret != pdata->device_id && count < MAX_POLL_COUNT) {

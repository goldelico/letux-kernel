...
static int fpd3_serdes_setup_aliases(struct i2c_client *client)
{
	...
	for (i = 0; i < data->num_slaves; i++) {
		...
		dev_err(&client->dev, "Registered alias 0x%2x for 0x%02x",
...
static u16 __get_alias_addr(struct i2c_adapter *adap, u16 addr)
{
	struct i2c_client *client = i2c_get_adapdata(adap);
	struct fpd3_serdes_data *data = i2c_get_clientdata(client);
	int i;
...
static struct i2c_adapter *__get_alias_adapter(struct i2c_adapter *adap)
{
	 ...
	 * used for communicating using the alias */
			...
			struct i2c_msg msgs[], int num)
{
	...
	struct i2c_msg temp[FPD3_I2C_MSG_BUFFER_NUM], *new_msgs = NULL;
	u16 alias_addr;
	int i, ret;
	...
	if (num < FPD3_I2C_MSG_BUFFER_NUM)
		new_msgs = temp;
	else
		new_msgs = kzalloc(sizeof(struct i2c_msg) * num, GFP_KERNEL);
	if (new_msgs == NULL)
		return -ENOMEM;

	/* Copy the i2c_msgs into temporary buffer and translate the address */
	for (i = 0; i < num; i++) {
		alias_addr = __get_alias_addr(adap, msgs[i].addr);
		if (alias_addr == 0)
			return -ENODEV;
		new_msgs[i] = msgs[i];
		new_msgs[i].addr = alias_addr;
	}

	/* Issue the messages on the alias adapter */
	ret = i2c_transfer(alias_adap, new_msgs, num);
	if (num >= FPD3_I2C_MSG_BUFFER_NUM)
		kfree(new_msgs);

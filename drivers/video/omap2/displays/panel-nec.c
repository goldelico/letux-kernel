...
static int zoom_spi_probe(struct spi_device *spi)
{
	spi->mode = SPI_MODE_0;
...
static int zoom_spi_remove(struct spi_device *spi)
{
...
static int zoom_spi_suspend(struct spi_device *spi, pm_message_t mesg)
{
...
static int zoom_spi_resume(struct spi_device *spi)
{
...
static struct spi_driver zoom_spi_driver = {
	.probe           = zoom_spi_probe,
	.remove	= __devexit_p(zoom_spi_remove),
	.suspend         = zoom_spi_suspend,
	.resume          = zoom_spi_resume,
	.driver         = {
		.name   = "zoom_disp_spi",
		.bus    = &spi_bus_type,
		.owner  = THIS_MODULE,
...
{
	return spi_register_driver(&zoom_spi_driver);
}

static void __exit nec_lcd_exit(void)
{
	return spi_unregister_driver(&zoom_spi_driver);
}

...
static int pico_i2c_read(int reg)
{
	...
	return (int)data[3] | ((int)(data[2]) << 8) |
...
int dpp2600_config_splash(int image_number)
{
	...
	printk(KERN_INFO "dpp2600 config splash");
...
static int picoDLP_panel_enable(struct omap_dss_device *dssdev)
{
	...
	modify_pico_register(display_control_reg + DSI_DIV2,
		0xFF | (0XFF << DSI_DIV_LCD),
		(1 << DSI_DIV_LCD) | (4 << DSI_DIV_PCD));
	/* LCD output Enabled */

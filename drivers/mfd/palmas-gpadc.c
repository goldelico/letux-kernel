...
#define GPADC_SLAVE		1
#define GPADC_TRIM_SLAVE	2
		...
		unsigned int *dest)
{
	...
	return regmap_read(palmas->regmap[GPADC_SLAVE], addr, dest);
}
		...
		unsigned int data)
{
	...
	return regmap_write(palmas->regmap[GPADC_SLAVE], addr, data);
}
		...
		u8 *dest, size_t count)
{
	...
	return regmap_bulk_read(palmas->regmap[GPADC_TRIM_SLAVE], addr, dest,
			count);
}
				...
				struct palmas_gpadc_result *res)
{
	...
	/*
	 * Currently there is an unexplained hardware issue with software
	 * conversion where the IRQ is never generated. Setting the RT_CONV_EN
	 * is for now a workaround until hardware guys figure out the issue.
	 */
	reg = PALMAS_GPADC_RT_SELECT_RT_CONV_EN;
	addr = PALMAS_BASE_TO_REG(PALMAS_GPADC_BASE, PALMAS_GPADC_RT_SELECT);
	ret = regmap_update_bits(gpadc->palmas->regmap[GPADC_SLAVE], addr, reg,
			reg);
	if (ret)
		return ret;
 ...
 * @return error
 */
int palmas_gpadc_bat_removal(struct palmas_gpadc *gpadc, int enable)
{
	unsigned int reg;
	int ret = 0;
	...
	reg &= ~PALMAS_GPADC_CTRL1_BAT_REMOVAL_DET;
	enable <<= PALMAS_GPADC_CTRL1_BAT_REMOVAL_DET_SHIFT;
	enable &= PALMAS_GPADC_CTRL1_BAT_REMOVAL_DET;
	reg |= enable;
...
EXPORT_SYMBOL(palmas_gpadc_bat_removal);
...
printk("pdata=%p\n", pdata);
	...
	gpadc = devm_kzalloc(&pdev->dev, sizeof(struct palmas_gpadc), GFP_KERNEL);
	if (!gpadc)
		return -ENOMEM;

	gpadc->palmas_cal_tbl = devm_kzalloc( &pdev->dev,

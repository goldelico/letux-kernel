...
struct ov9655_reg {
	u8 addr;
	u8 value;
	/* mask for bit to be changed
	 * note: 0x00 is treated as 0xff
	 */
	u8 mask;
};
				...
				const struct ov9655_reg *regs, const int n)
{
	...
	for (i = 0; i < n; i++) {
		ret = ov9655_update_bits(client, regs[i].addr, regs[i].mask ? regs[i].mask : 0xff, regs[i].value);
		if (ret < 0)
			return ret;
	}
	return 0;

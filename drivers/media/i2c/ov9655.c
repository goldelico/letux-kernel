...
static /* inline */ int ov9655_set_image_size_regs(struct i2c_client *client, int upperreg, int lowerreg, int mask, int value)
{
	int ret;
...
static /* inline */ int ov9655_set_format_regs(struct i2c_client *client, int swap, int uyvy, int tslb, int rgb)
{
	int ret;
	...
	ret = ov9655_update_bits(client, REG_COM15, COM15_RGB_MASK, rgb);	// RGB565
...
{ /* called by ov9655_s_stream() */
	...
	ret = ov9655_write(client, REG_AREF3, is_sxga ? 0xf9 : 0xfa);	/* magic */
	ret = ov9655_write(client, REG_HV, is_sxga ? 0x02 : 0x0a);	/* magic */
	ret = ov9655_update_bits(client, REG_COM19, 0x80, is_sxga ? 0 : 0x80);	/* magic */
	ret = ov9655_write(client, REG_REF_A9, is_sxga ? 0x8d : 0xef);	/* magic */
	ret = ov9655_update_bits(client, REG_COM16, COM16_SCALING, ov9655_framesizes[i].scaling);

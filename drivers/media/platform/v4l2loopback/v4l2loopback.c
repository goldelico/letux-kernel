...
struct v4l2_loopback_device {
	...
	int gen_timestamp;	/*whether generate timestamp by kernel or not.*/


	struct list_head queued_list;	/*list for capture queued buffer.*/
	struct list_head done_list;	/*list for output queued buffer.*/
			  ...
			  struct v4l2_requestbuffers *b)
{
	...
	INIT_LIST_HEAD(&dev->queued_list);
	INIT_LIST_HEAD(&dev->done_list);
...
static int vidioc_qbuf(struct file *file, void *fh, struct v4l2_buffer *buf)
{
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
		...
		set_queued(b);
		list_add_tail(&b->list_head, &dev->queued_list);
		spin_unlock_irqrestore(&dev->done_lock, flags);

		dprintkrw("[capture] qbuf: %d, b: %x, add_to queued_list.\n", buf->index, b);
		return 0;
	case V4L2_BUF_TYPE_VIDEO_OUTPUT:

		if(!(b->buffer.flags & V4L2_BUF_FLAG_QUEUED)) {
			printk("[output] qbuf [%d] without V4L2_BUF_FLAG_QUEUED.\n", b->buffer.index);
			return -EINVAL;
		}

		v4l2l_get_timestamp(&b->buffer);
		b->buffer.bytesused = buf->bytesused;
		b->buffer.sequence = dev->output_sequence ++;

		spin_lock_irqsave(&dev->done_lock, flags);
		set_done(b);
		list_add_tail(&b->list_head, &dev->done_list);

		dprintkrw("[output] qbuf: %d, b: %x, add_to done_list\n", b->buffer.index, b);
		spin_unlock_irqrestore(&dev->done_lock, flags);
		    ...
		    struct v4l2_loopback_opener *opener)
{
	...
	ret =  (!list_empty(&dev->done_list) || dev->ready_for_output);
...
static int vidioc_dqbuf(struct file *file, void *fh, struct v4l2_buffer *buf)
{
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
		...
		wait_event_interruptible(dev->read_event, (!list_empty(&dev->done_list) || dev->ready_for_output));
		if(dev->ready_for_output) {
			printk("output is not streaming!\n");
			return -EINVAL;
		}

		spin_lock_irqsave(&dev->done_lock, flags);

		b = list_first_entry_or_null(&dev->done_list, struct v4l2l_buffer, list_head);

		if(!b) {
			printk("no available buffer on done_list.!\n");
			spin_unlock_irqrestore(&dev->done_lock, flags);
			return -EINVAL;
		}

		/*remove from done list*/
		list_del(&b->list_head);

		unset_flags(b);
		*buf = b->buffer; //dev->buffers[index].buffer;
		spin_unlock_irqrestore(&dev->done_lock, flags);

		dprintkrw("[capture] [dbuf: %x index: %d - %d userptr: %x]: tv_sec: %d, tv_usec: %d, sequence: %d\n", b, index, buf->index, buf->m.userptr, buf->timestamp.tv_sec, buf->timestamp.tv_usec, buf->sequence);
		return 0;
	case V4L2_BUF_TYPE_VIDEO_OUTPUT:

		spin_lock_irqsave(&dev->done_lock, flags);
		b = list_first_entry_or_null(&dev->queued_list, struct v4l2l_buffer, list_head);
		if(!b) {
			spin_unlock_irqrestore(&dev->done_lock, flags);
			return -EINVAL;
		}

		dprintkrw("[output] dqbuf get b: %x, b->buffer.index: %d, b->buffer.flags: %x\n", b, b->buffer.index, b->buffer.flags);
...
#ifdef USING_DMA_ALLOC_BUFFER
		// TODO:
		addr_p = (unsigned long)dev->image_paddr + (vma->vm_pgoff << PAGE_SHIFT);
#endif
	}

#ifdef USING_DMA_ALLOC_BUFFER
	vma->vm_pgoff = 0;
	ret = dma_mmap_noncoherent(&dev->vdev->dev, vma, addr, addr_p, size);
	vma->vm_flags           |= VM_DONTEXPAND | VM_DONTDUMP;
	//printk("-----%s, %d addr:%x, dma_addr: %x size: %d, start:%x vma->vm_pgoff: %d, ret: %d\n", __func__, __LINE__, addr, addr_p, size, start, vma->vm_pgoff, ret);
#else
	while (size > 0) {
		struct page *page;
				  ...
				  size_t count, loff_t *ppos)
{
	 ...
	 * for capture stream:
	 *
	 * 1. get done buffer from done list?
	 *
	 * 2. memcpy from done buffer.
	 *
	 * 3. put it back to queued list?
	 *
	 *
	 * */
	wait_event_interruptible(dev->read_event, !list_empty(&dev->read_list));

	spin_lock_irqsave(&dev->read_write_lock, flags);

	b = list_first_entry_or_null(&dev->read_list, struct v4l2l_buffer, list_head);

	if(!b) {
		printk("error no buffer on read list.!\n");
		//TODO:
	}
	list_del(&b->list_head);

	spin_unlock_irqrestore(&dev->read_write_lock, flags);
	...
	// give back buffer to write list.
	list_add_tail(&b->list_head, &dev->write_list);
#endif
				   ...
				   size_t count, loff_t *ppos)
{
	...
	if (copy_from_user((void *)(dev->image + b->buffer.m.offset), (void *)buf,
			   count)) {
		printk(KERN_ERR
	...
	b->buffer.sequence = dev->output_sequence ++;	//TODO:
	set_done(b);
...
static int free_buffers(struct v4l2_loopback_device *dev)
{
	...
	if (dev->image) {
#ifdef USING_DMA_ALLOC_BUFFER
		dma_free_noncoherent(&dev->vdev->dev, dev->imagesize, dev->image, dev->image_paddr);
#else
		vfree(dev->image);
#endif
		dev->image = NULL;
...
#ifdef USING_DMA_ALLOC_BUFFER
	dev->image = dma_alloc_noncoherent(&dev->vdev->dev, dev->imagesize, &dev->image_paddr, GFP_KERNEL);
#else
...
#if 0
		/*dev->buffers 作为output 和 capture 共同操作的数据, 是不是应该加锁处理????*/
		b->sequence = 0;

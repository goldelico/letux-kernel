...
#define TCA6507_LS_LED_ON	0x4	/* Output LOW */
#define TCA6507_LS_LED_OFF	0x0	/* Output HI-Z */
#define TCA6507_LS_BLINK0	0x6	/* Blink at PWM0 rate */
#define TCA6507_LS_BLINK1	0x7	/* Blink at PWM1 rate */
 ...
 * the appropriate 3-bit state value set for the given LED number (0-3).
 */
static inline u32 tca6507_ledsel(u32 oldval, int led_num, int state)
{
	oldval &= ~(0x111 << led_num);
	oldval |= ((state & 0x01) >> 0) << (0+led_num);
	oldval |= ((state & 0x02) >> 1) << (8+led_num);
	oldval |= ((state & 0x04) >> 2) << (16+led_num);
	return oldval;
}
 ...
 */
static void tca6507_write_pwm(struct i2c_client *client, int n, u8 val)
{
//	struct tca6507_led *tca6507 = i2c_get_clientdata(client);
...
static void tca6507_led_work(struct work_struct *work)
{
	...
	default:
		 ...
		 */
		tca6507_write_pwm(tca6507->client, 1, 255-tca6507->brightness);
		ls = tca6507_ledsel(ls, ls_led, TCA6507_LS_BLINK1);
		break;
	}

...
static int is31fl319x_write(struct is31fl319x_chip *is31, u8 reg, u8 byte)
{
	...
	dev_err(&is31->client->dev, "write %02x to reg %02x\n", byte, reg);
	return i2c_smbus_write_byte_data(cl, reg, byte);
}

#if 0
static int is31fl319x_read(struct is31fl319x_chip *is31, u8 reg, u8 *byte)
{
	struct i2c_client *cl = is31->client;
	if (reg >= IS31FL319X_REG_CNT)
		return -EIO;
	*byte = is31->reg_file[reg];	/* read crom cache (can't read chip) */
	return 0;	/* pretend the read was ok */
}
#endif
...
static void is31fl319x_work(struct work_struct *work)
{
	...
	dev_err(&is31->client->dev, "work called\n");
	...
	dev_err(&is31->client->dev, "write to chip\n");

	for (i = 0; i < NUM_LEDS; i++) {
		struct led_classdev *led = &is31->leds[i].led_cdev;
		u8 ctl, bit;

		dev_err(&is31->client->dev, "set brightness %u for led %u\n", led->brightness, i);

		/* update brightness register */
		is31fl319x_write(is31, IS31FL319X_PWM1 + i, led->brightness);

		/* update output enable bit */
		ctl = IS31FL319X_CTRL1 + (i / 6);
		bit = 1 << ((i + i / 3) % 6);
		dev_err(&is31->client->dev, "led %d -> register %d and bit %d\n", i, ctl, bit);
		is31fl319x_clear_set(is31, ctl, bit, led->brightness > LED_OFF ? bit : 0);
	}

	/* check if any PWM is enabled or all outputs are now off */
	if (is31->reg_file[IS31FL319X_CTRL1] | is31->reg_file[IS31FL319X_CTRL2]) {
		dev_err(&is31->client->dev, "power up\n");
		is31fl319x_write(is31, IS31FL319X_DATA_UPDATE, 0x00);	/* update */
		is31fl319x_write(is31, IS31FL319X_SHUTDOWN, 0x01);	/* enable */
	} else {
		dev_err(&is31->client->dev, "power down\n");
		is31fl319x_write(is31, IS31FL319X_SHUTDOWN, 0x00);	/* shut down */
	}

	dev_err(&is31->client->dev, "work done\n");
				   ...
				   enum led_brightness brightness)
{
	...
	if (led->led_cdev.brightness != brightness) {
			     ...
			     unsigned long *delay_off)
{
	...
	if (!is31->work_scheduled) {
		schedule_work(&is31->work);
		is31->work_scheduled = true;
		printk("scheduled for blink %u %u\n", *delay_on, *delay_off);
	}

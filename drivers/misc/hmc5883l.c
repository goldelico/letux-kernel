...
#define HMC5883L_ID_REG_LENGTH	0x03	/* Registers A, B, C*/
#define HMC5883L_ID_STRING		"H43"

/* Addresses to scan: 0x1E */
static const unsigned short normal_i2c[] = { HMC5883L_I2C_ADDRESS,
							I2C_CLIENT_END };
...
struct hmc5883l_data {
	enum HMC5883L_OUTPUT_RATE	rate;
	enum HMC5883L_OPERATING_MODE	mode;
	enum HMC5883L_CONVERSION_MODE	conversion;
	enum HMC5883L_RANGE		range;
	enum HMC5883L_STATUS		status;
	s16				data[3];
};

static void hmc5883l_init_client(struct i2c_client *client);


static s32 hmc5883l_get_status(struct i2c_client *client)
{
	return i2c_smbus_read_byte_data(client, HMC5883L_STATUS_REG);
}

static s32 hmc5883l_get_conversion_mode(struct i2c_client *client)
{
	/* The lower two bits contain the current conversion mode */
	return i2c_smbus_read_byte_data(client, HMC5883L_MODE_REG) & 0x03;
}

static s32 hmc5883l_set_conversion_mode(struct i2c_client *client,
				       enum HMC5883L_CONVERSION_MODE conversion)
{
	return i2c_smbus_write_byte_data(client, HMC5883L_MODE_REG,
							conversion & 0x03);
}

static s32 hmc5883l_get_operating_mode(struct i2c_client *client)
{
	return i2c_smbus_read_byte_data(client, HMC5883L_CONFIG_REG_A) & 0x03;
}

static s32 hmc5883l_set_operating_mode(struct i2c_client *client,
				      enum HMC5883L_OPERATING_MODE mode)
{
	struct hmc5883l_data *data = i2c_get_clientdata(client);
	return  i2c_smbus_write_byte_data(client, HMC5883L_CONFIG_REG_A,
						(mode & 0x03) + data->rate);
}

static s32 hmc5883l_get_rate(struct i2c_client *client)
{
	/* The output data rate is specified by bits 3 and 4 */
	return i2c_smbus_read_byte_data(client, HMC5883L_CONFIG_REG_A) & 0x1C;
}

static s32 hmc5883l_set_rate(struct i2c_client *client,
				enum HMC5883L_OUTPUT_RATE rate)
{
	struct hmc5883l_data *data = i2c_get_clientdata(client);
	if (rate == RATE_0)
		/* if the data rate is 0, switch to single conversion mode */
		hmc5883l_set_conversion_mode(client, CONVERSION_SINGLE);
	else
		/* else switch to continuous conversion mode */
		hmc5883l_set_conversion_mode(client, CONVERSION_CONTINUOUS);
		return i2c_smbus_write_byte_data(client, HMC5883L_CONFIG_REG_A,
							data->mode + rate);
}

static s32 hmc5883l_get_range(struct i2c_client *client)
{
	return i2c_smbus_read_byte_data(client, HMC5883L_CONFIG_REG_B);
}

static s32 hmc5883l_set_range(struct i2c_client *client, enum HMC5883L_RANGE range)
{
	return i2c_smbus_write_byte_data(client, HMC5883L_CONFIG_REG_B, range);
}

static s32 hmc5883l_read_measurements(struct i2c_client *client, s16 *data)
{
	/* read all data output registers */
	return i2c_smbus_read_i2c_block_data(client, HMC5883L_DATA_OUT_X_MSB_REG,
							0x06, (u8 *)data);
}
...
static DEVICE_ATTR(values, S_IRUGO, show_value, NULL);
...
{
	struct i2c_client *client = to_i2c_client(dev);
	s32 status = hmc5883l_get_status(client);
...
static ssize_t show_conversion_mode(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	switch (hmc5883l_get_conversion_mode(client)) {
...
static ssize_t show_mode(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	switch (hmc5883l_get_operating_mode(client)) {
	case MODE_NORMAL:
...
static ssize_t set_mode(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (hmc5883l_set_operating_mode(client, mode) == -EINVAL)
		return -EINVAL;
	data->mode = mode;
	return count;
}
...
{
	struct i2c_client *client = to_i2c_client(dev);
	switch (hmc5883l_get_rate(client)) {
...
static ssize_t set_rate(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct hmc5883l_data *data = i2c_get_clientdata(client);
	enum HMC5883L_OUTPUT_RATE rate = RATE_0;
	...
	if (hmc5883l_set_rate(client, rate) == -EINVAL)
		return -EINVAL;
	data->rate = rate;
	return count;
}
...
{
	struct i2c_client *client = to_i2c_client(dev);
	s32 range = hmc5883l_get_range(client);
	switch (range) {
...
static ssize_t set_range(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct hmc5883l_data *data = i2c_get_clientdata(client);
	enum HMC5883L_RANGE range = RANGE_1_0;
	unsigned long param = 0;
	strict_strtoul(buf, 10, &param);
	...
	if (hmc5883l_set_range(client, range) == -EINVAL)
		return -EINVAL;
	data->range = range;
	return count;
}
...
static const struct attribute_group hmc5883l_attr_group = {
	.attrs = hmc5883l_attributes,
};


static int hmc5883l_detect(struct i2c_client *client,
			  struct i2c_board_info *info)
{
	unsigned char id_str[HMC5883L_ID_REG_LENGTH];

	if (client->addr != HMC5883L_I2C_ADDRESS)
		return -ENODEV;

	if (i2c_smbus_read_i2c_block_data(client, HMC5883L_ID_REG_A,
				HMC5883L_ID_REG_LENGTH, id_str)
			!= HMC5883L_ID_REG_LENGTH)
		return -ENODEV;

	if (0 != strncmp(id_str, HMC5883L_ID_STRING, HMC5883L_ID_REG_LENGTH))
		return -ENODEV;

	strlcpy(info->type, "hmc5883l", I2C_NAME_SIZE);
	return 0;
}
			 ...
			 const struct i2c_device_id *id)
{
	...
	data = kzalloc(sizeof(struct hmc5883l_data), GFP_KERNEL);
	if (!data) {
		err = -ENOMEM;
		goto exit;
	}
	...
	/* Initialize the HMC5883L chip */
	hmc5883l_init_client(client);

	/* Register sysfs hooks */
	err = sysfs_create_group(&client->dev.kobj, &hmc5883l_attr_group);
	if (err)
		goto exit_free;
...
{
	hmc5883l_set_conversion_mode(client, CONVERSION_SLEEP); /* set chip to sleep mode */
	sysfs_remove_group(&client->dev.kobj, &hmc5883l_attr_group);
	kfree(i2c_get_clientdata(client));
	return 0;
}

/* Called when we have found a new HMC5883L. */
static void hmc5883l_init_client(struct i2c_client *client)
{
	struct hmc5883l_data *data = i2c_get_clientdata(client);
	hmc5883l_set_operating_mode(client, data->mode);
	hmc5883l_set_range(client, data->range);
	hmc5883l_set_rate(client, data->rate);
	hmc5883l_set_conversion_mode(client, data->conversion);
	printk(KERN_INFO "HMC5883L initialized\n");
}

static int hmc5883l_suspend(struct i2c_client *client, pm_message_t mesg)
{
	hmc5883l_set_conversion_mode(client, CONVERSION_SLEEP);
	printk(KERN_INFO "HMC5883L suspended\n");
	return 0;
}

static int hmc5883l_resume(struct i2c_client *client)
{
	struct hmc5883l_data *data = i2c_get_clientdata(client);
	hmc5883l_set_conversion_mode(client, data->conversion);
	printk(KERN_INFO "HMC5883L resumed\n");
	return 0;
}

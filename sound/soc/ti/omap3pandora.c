...
#define AMP_GPIO_NAME			"opnd,amp-gpio"
	...
	struct snd_pcm_hw_params *params)
{
	...
	ret = snd_soc_dai_set_sysclk(twl4030_rtd->codec_dai,
...
static int omap3pandora_probe(struct platform_device *pdev)
{
	...
	if (!ctx) {
		dev_err(dev, "cannot allocate space for runtime data\n");
		return -ENOMEM;
	}
	ctx->amp_gpio = -1;
...
static const struct of_device_id omap3pandora_of_match[] = {
	{ .compatible = "opnd,omap3pandora-sound", },
	{ },
};

 ...
 */
static struct snd_dmaengine_dai_dma_data omap_mcasp_dai_dma_params[] = {
	{
		/* TODO: this might fail... We should get them as resource */
		.addr = OMAP44XX_MCASP_DAT_BASE + OMAP_MCASP_TXBUF0_REG,
	},
};
...
static const struct snd_soc_component_driver omap_mcasp_component = {
	.name		= "omap-mcasp",
};
...
static int asoc_mcasp_probe(struct platform_device *pdev)
{
	...
	ret = snd_soc_register_component(&pdev->dev, &omap_mcasp_component,
					 &omap_mcasp_dai, 1);
	if (ret < 0)
		goto err_dai;

...
{
	struct mscaler_device *mscaler = v4l2_get_subdevdata(sd);
	mscaler->sensor_id = val;
	if(val == 0)
		mscaler->iobase = 0xb3316000;
	else if(val == 1)
		mscaler->iobase = 0xb3317000;
	return 0;
}
...
static int mscaler_stream_enable(struct mscaler_device *mscaler, int ch)
{
	...
	attr.fcrop_x = 0;
	attr.fcrop_y = 0;
	attr.fcrop_width = input_fmt->format.width;
	attr.fcrop_height = input_fmt->format.height;
	attr.scaler_width = output_fmt->format.width;
	attr.scaler_height = output_fmt->format.height;
	attr.crop_x = mscaler->crop.c.left;
	attr.crop_y = mscaler->crop.c.top;
	attr.crop_width = mscaler->crop.c.width;
	attr.crop_height = mscaler->crop.c.height;

	if(mscaler->sensor_id == 0)
		tisp_channel_main_attr_set(ch, &attr);
	else if(mscaler->sensor_id == 1)
		tisp_channel_sec_attr_set(ch, &attr);

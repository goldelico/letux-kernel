...
static int rot_queue_setup(struct vb2_queue *vq,
...
static int rot_reqdesc(struct ingenic_rot_ctx *ctx)
{
	...
	vfd->dev.coherent_dma_mask = DMA_BIT_MASK(32);
...
static void device_run(void *prv)
{
	...
	rot_set_src_desc(dev, ingenic_vb2_dma_contig_plane_dma_addr(&src->vb2_buf, 0));

	rot_set_dst_cfg(dev, &ctx->out);
	rot_set_dst_desc(dev, ingenic_vb2_dma_contig_plane_dma_addr(&dst->vb2_buf, 0));
...
static irqreturn_t rot_irq_handler(int irq, void *prv)
{
	...
	time_last = ktime_get();
	interval_in_ns = time_last - time_now;
...
#endif
	dev->alloc_ctx = ingenic_vb2_dma_contig_init_ctx(&pdev->dev);

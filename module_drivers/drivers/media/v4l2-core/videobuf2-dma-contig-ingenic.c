...
static void vb2_dc_put(void *buf_priv)
{
	...
	if(buf->rsvmem)
	{
		...
		dma_free_noncoherent(buf->dev, buf->size, buf->vaddr, buf->dma_addr, DMA_BIDIRECTIONAL);
				...
				enum dma_data_direction dma_dir, gfp_t gfp_flags)
{
	...
	if(!buf->vaddr)
	{
		buf->vaddr = dma_alloc_noncoherent(dev, size, &buf->dma_addr, DMA_BIDIRECTIONAL,
	...
	buf->handler.refcount = (refcount_t *)&buf->refcount;
...
static int vb2_dc_dmabuf_ops_attach(struct dma_buf *dbuf, struct dma_buf_attachment *dbuf_attach)
{
...
void *ingenic_vb2_dma_contig_init_ctx(struct device *dev)
{
	...
	conf->dev = dev;
	if(reserve_size > 0){
		conf->reserve_addr = dma_alloc_noncoherent(dev, reserve_size * 1024, &conf->reserve_phy_addr, DMA_BIDIRECTIONAL, GFP_KERNEL);

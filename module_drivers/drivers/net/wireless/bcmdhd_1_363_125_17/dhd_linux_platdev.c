...
#ifdef CUSTOMER_OOB
extern int dhd_init_gpio_wlan_hostwake(void);
extern void dhd_deinit_gpio_wlan_hostwake(void);
#endif
...
#endif /* CONFIG_DTS */
/*power on bcm43455---->*/
	err = dhd_customer_gpio_wlan_ctrl(adapter, on);
	if (msec && !err)
		OSL_SLEEP(msec);
...
static int wifi_ctrlfunc_register_drv(void)
{
	...
	if (dts_enabled) {
		...
		wifi_plat_dev_probe_ret = dhd_init_gpio_wlan_hostwake();
